namespace NServiceBus.Transport.AzureServiceBus.AcceptanceTests.Sending
{
    ...
    {
        [Test]
        public async Task Should_move_message_to_error_queue_single()
        {
            var context = await Scenario.Define<Context>()
                                        .WithEndpoint<Sender>(b => b.When(session => session.SendLocal(new KickOffMessage())).DoNotFailOnErrorMessages())
                                        .WithEndpoint<Receiver>()
                                        .WithEndpoint<ErrorSpy>()
                                        .Done(c => c.MessageMovedToTheErrorQueue || c.MessageIdsReceived.Any())
                                        .Run();

            // no messages should leak
            Assert.IsEmpty(context.MessageIdsReceived, "Detected ghost messages");
            Assert.IsTrue(context.MessageMovedToTheErrorQueue, "Message failed but did not move to the error queue");
        }
        ...
        public class Context : ScenarioContext
        {
            ...
            public int NrOfMessagesReceived => MessageIdsReceived.Count;
        }
